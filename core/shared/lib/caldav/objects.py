...
from .lib import error, vcal, url
from .lib.url import URL
from .elements import dav, cdav
...
class Principal(DAVObject):
    ...
    def addressbook(self):
        abhs = self.get_properties([cdav.AddressbookHomeSet()])
        addressbook_home_set_url = abhs['{urn:ietf:params:xml:ns:carddav}addressbook-home-set']
        addressbook_home_set_url = URL.objectify(addressbook_home_set_url)

        addressbook_home_set = DAVObject(url=addressbook_home_set_url.path, parent=self)

        addressbooks = addressbook_home_set.children(cdav.Addressbook.tag)

        first_addressbook = Addressbook(client=self.client, url=addressbooks[0][0], parent=addressbook_home_set)
        return first_addressbook
...
class CalendarObjectResource(DAVObject):
    ...
    def _create(self, data, id=None, path=None):
        if id is None and path is not None and str(path).endswith('.ics'):
            ...
            pattern = re.compile(r'UID:(?P<id>EVENTURE-[0-9]+)')
            id = pattern.search(data).group('id')
...
# FOR CARDDAV
class Contact(DAVObject):
    pass


class Addressbook(DAVObject):
    def contacts(self):
        all = []
        data = cdav.AddressData()
        prop = dav.Prop() + data
        root = cdav.AddressbookQuery() + [prop, ]

        response = self._query(root, 1, query_method='report')
        results = self._handle_prop_response(response, props=[cdav.AddressData()])

        for r in results:
            all.append(Contact(self.client, url=self.url.join(r), data=results[r][cdav.AddressData.tag], parent=self))
        return all
